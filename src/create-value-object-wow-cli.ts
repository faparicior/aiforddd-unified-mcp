...
program
    .name('ddd-create-value-object-wow')
    .description('Analyze value object patterns in a code manifest and generate a DDD Domain Layer Value Object WoW document using Claude CLI')
    .version('1.0.0')
    ...
    .action(async (options) => {
        try {
            ...
            // {{manifest_path}}/wow/ddd-value-object-wow.md
            ...
            const promptContent = promptManager.getPromptContent('create-value-object-wow', promptArgs)
            ...
            const tempPromptFile = resolve(process.cwd(), '.ddd-vo-wow-prompt-tmp.md')
            writeFileSync(tempPromptFile, promptContent, 'utf-8')

            console.log(`Executing Claude workflow to generate Value Object WoW document for: ${manifestFile}`)
            ...
            const outputFile = join(wowOutputDir, 'ddd-value-object-wow.md')
            ...
            console.error('Error running create-value-object-wow workflow:', e.message || e)
            process.exit(1)
        }
    })
